...
class Topo:
    ...
    def __init__(self):
        ...
        self.symbols = ['GC', 'BRR', 'ES', 'NQ']
        #self.symbols = ['NQ']

        self.exchanges = ['NYMEX', 'CMECRYPTO', 'GLOBEX', 'GLOBEX']
        #self.exchanges = ['GLOBEX']

        self.data_type = 'Bid_Ask'
        self.counter = 0
        self.data = []
        self.re_run = True    # False Default
        self.run = 0
    ...
    def credentials(self):
        ...
        self.telegram_token = credentials_json['telegram']['bot_token_BA']
        self.telegram_chat = credentials_json['telegram']['bot_chatID_BA']
    ...
    def db_connect(self):
        ...
        store = Arctic('db')
        self.library = store['Futures_Historical_Ticks_BA']
    ...
    def save_data(self):
        ...
        final = final.rename(columns={'Time': 'Date', 'Pricebid' : 'Bid', 'Priceask' : 'Ask', 'Sizebid' : 'Bid_Volume', 'Sizeask' : 'Ask_Volume' })
        final.drop(columns= ['Tickattribbidask'], inplace=True)
        ...
        final.to_csv('Topo_Data/{}/{}_{}-{}_ticks_BA.csv'.format(self.ticket, self.ticket, init_date , end_date)) #Session
        final.to_csv('Topo_Data/{}/{}_master_BA.csv'.format(self.ticket, self.ticket), mode='a', header=False) #Master
        print('Appending to Arctic...')
        self.library.append(self.ticket, final, metadata={'source': 'Qs'})
        
    def digging(self):
        '''Starts the topo'''
        self.current_time = datetime.now(self.tz)
        while not((self.current_time.weekday() == 4) & (self.current_time.hour > 23)): #Be active during the week, finish at market close
            ...
            if (self.current_time.weekday() <= 4) & (self.current_time.hour == 19) & (self.current_time.minute <= 2):
                self.current_time = self.current_time.replace(hour=17, minute=0, second=0, microsecond=0)
